...
function StorageBadge({ type }) {
  if (type === "s3") return (
    <span className="inline-flex items-center gap-1 text-xs text-[color:var(--primary)] border border-[color:var(--primary)]/40 px-2 py-0.5 rounded-full font-medium">
      <Cloud className="w-3 h-3" /> S3
    </span>
  );
  if (type === "local") return (
    <span className="inline-flex items-center gap-1 text-xs text-[color:var(--text-muted)] border border-[color:var(--border)] px-2 py-0.5 rounded-full font-medium">
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl border border-[color:var(--border)] flex items-center justify-center">
            <Database className="w-5 h-5 text-[color:var(--primary)]" />
          </div>
          <div>
            <h1 className="text-xl font-bold text-[color:var(--text)]">Database Backups</h1>
            <p className="text-xs text-[color:var(--text-muted)] mt-0.5">Automated daily backups · 30-day retention</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={load}
            disabled={loading}
            className="border border-[color:var(--border)] text-[color:var(--text-muted)] px-4 py-2 rounded-lg text-sm hover:text-[color:var(--text)] hover:border-[color:var(--border-strong)] transition-colors flex items-center gap-1.5"
          >
          ...
          <button
            ...
            disabled={triggering}
            className="bg-[color:var(--primary)] text-[color:var(--primary-contrast)] px-4 py-2 rounded-lg text-sm font-semibold hover:bg-[color:var(--primary-hover)] transition-colors flex items-center gap-1.5 disabled:opacity-60"
          >
      ...
      {/* Trigger feedback */}
      {triggerMsg && (
        <div className="mb-4 px-4 py-3 rounded-lg border border-[color:var(--primary)]/30 text-sm text-[color:var(--primary)]">
          {triggerMsg}
        </div>
      )}

      {/* Recovery controls */}
      <div className="border border-[color:var(--border)] rounded-lg p-4 mb-6">
        <div className="flex items-center justify-between gap-3 mb-3">
          <div>
            <p className="text-sm font-semibold text-[color:var(--text)]">Workspace Recovery</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div>
            <label className="block text-xs font-medium text-[color:var(--text-muted)] mb-1">Source Snapshot DB URL (optional override)</label>
            <input
          ...
          <div className="flex items-end gap-3">
            <button
              ...
              className="border border-[color:var(--border)] text-[color:var(--text-muted)] px-4 py-2 rounded-lg text-sm hover:text-[color:var(--text)] hover:border-[color:var(--border-strong)] transition-colors inline-flex items-center gap-1"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 mb-6">
          {[
            ...
            <div key={label} className="border border-[color:var(--border)] rounded-lg p-4">
              <p className="text-xs font-semibold text-[color:var(--text-muted)] uppercase tracking-wide mb-1">{label}</p>
      ...
      {/* Error state */}
      {error && (
        <div className="mb-4 px-4 py-3 rounded-lg border border-red-500/30 text-sm text-red-500">
          {error}
        </div>
      )}
      ...
      <div className="border border-[color:var(--border)] rounded-lg overflow-hidden">
        <div className="px-4 py-3 border-b border-[color:var(--border)]">
          <p className="text-sm font-semibold text-[color:var(--text)]">Backup History</p>
        </div>

        {loading ? (
          <div className="py-12 text-center text-[color:var(--text-muted)] text-sm">Loading…</div>
        ) : logs.length === 0 ? (
          <div className="py-12 text-center text-[color:var(--text-muted)] text-sm">No backups yet.</div>
        ) : (
              ...
              <thead>
                <tr className="border-b border-[color:var(--border)]">
                  {["Status", "File", "Size", "Storage", "Triggered By", "Started At", "Duration", "Error"].map((h) => (
                    <th key={h} className="px-4 py-2.5 text-left text-xs font-semibold text-[color:var(--text-muted)] uppercase tracking-wide whitespace-nowrap">
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {logs.map((log) => {
                  ...
                  return (
                    <tr key={log.id} className="hover:bg-[var(--surface-soft)] transition-colors">
                      ...
                      <td className="px-4 py-3 text-[color:var(--text-muted)] font-mono text-xs max-w-[200px] truncate" title={log.file_name}>
                        {log.file_name || "—"}
                      </td>
                      <td className="px-4 py-3 text-[color:var(--text)] whitespace-nowrap">{fmtBytes(log.file_size_bytes)}</td>
                      <td className="px-4 py-3"><StorageBadge type={log.storage_type} /></td>
                      <td className="px-4 py-3 text-[color:var(--text)] capitalize">{log.triggered_by || "—"}</td>
                      <td className="px-4 py-3 text-[color:var(--text-muted)] whitespace-nowrap">{fmtDate(log.started_at)}</td>
                      <td className="px-4 py-3 text-[color:var(--text)]">{duration}</td>
      ...
      {activeRecoveryJob && (
        <div className="border border-[color:var(--border)] rounded-lg p-4 mt-6">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm font-semibold text-[color:var(--text)]">Live Recovery Progress</p>
          ...
          <div className="mt-2 flex flex-wrap gap-3 text-xs text-[color:var(--text-muted)]">
            ...
            <span>Table: <span className="text-[color:var(--text)] font-medium">{activeRecoveryJob.current_table || "—"}</span></span>
          </div>

          {activeRecoveryJob.progress_message && (
            <div className="mt-3 px-3 py-2 rounded-lg border border-[color:var(--border)] text-xs text-[color:var(--text)]">
              {activeRecoveryJob.progress_message}
            </div>
          )}

          <div className="mt-3 rounded-lg border border-[color:var(--border)] overflow-hidden">
            <div className="px-3 py-2 text-xs font-semibold text-[color:var(--text-muted)] border-b border-[color:var(--border)]">
              Live Log
            </div>
            {liveEvents.length === 0 ? (
              <div className="px-3 py-3 text-xs text-[color:var(--text-muted)]">No live events yet.</div>
            ) : (
              <div className="max-h-44 overflow-y-auto">
                {liveEvents.map((evt, idx) => (
                  <div key={`${evt.at || "evt"}-${idx}`} className="px-3 py-2 text-xs border-b border-[color:var(--border)] last:border-b-0">
                    <p className="text-[color:var(--text)]">{evt.message || "—"}</p>
                    <p className="text-[color:var(--text-muted)] mt-0.5">
      ...
      {/* Recovery jobs */}
      <div className="border border-[color:var(--border)] rounded-lg overflow-hidden mt-6">
        <div className="px-4 py-3 border-b border-[color:var(--border)]">
          <p className="text-sm font-semibold text-[color:var(--text)]">Workspace Recovery Jobs</p>
        </div>
        {loading ? (
          <div className="py-12 text-center text-[color:var(--text-muted)] text-sm">Loading…</div>
        ) : recoveryJobs.length === 0 ? (
          <div className="py-12 text-center text-[color:var(--text-muted)] text-sm">No recovery jobs yet.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
              ...
              <tbody className="divide-y divide-[color:var(--border)]">
                {recoveryJobs.map((job) => (
                  <tr key={job.id} className="hover:bg-[var(--surface-soft)] transition-colors">
                    ...
                    <td className="px-4 py-3 text-[color:var(--text-muted)] whitespace-nowrap">{fmtDate(job.started_at || job.created_at)}</td>
                    <td className="px-4 py-3 text-[color:var(--text-muted)] whitespace-nowrap">{fmtDate(job.completed_at)}</td>
                    <td className="px-4 py-3 text-[color:var(--text)] whitespace-nowrap">{Number(job.progress_pct || 0).toFixed(1)}%</td>
                    <td className="px-4 py-3 text-[color:var(--text)]">{job.rows_scanned ?? 0}</td>
                    <td className="px-4 py-3 text-[color:var(--text)]">{job.rows_written ?? 0}</td>
                    <td className="px-4 py-3 text-[color:var(--text)] text-xs max-w-[250px] truncate" title={job.progress_message || ""}>
