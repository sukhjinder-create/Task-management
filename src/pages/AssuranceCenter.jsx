      ...
      <section className="grid gap-3 lg:grid-cols-2">
        {portfolios.map((portfolio) => (
          <article key={portfolio.id} className="rounded-[9px] border border-[color:var(--border)] bg-[var(--surface)] p-4">
            ...
            {portfolio.sla && <div className={`mt-3 rounded-[7px] px-3 py-2 text-[11px] ${portfolio.sla.healthy ? "bg-[var(--score-good)]/10 text-[color:var(--score-good)]" : "bg-[var(--score-danger)]/10 text-[color:var(--score-danger)]"}`}><div className="flex items-center justify-between gap-3"><span>{portfolio.sla.healthy ? "SLA healthy" : `${portfolio.sla.exceptions?.length || 0} SLA exception(s)`}</span>{portfolio.canManage && <button type="button" className="font-semibold underline" onClick={() => run(() => api.post(`/assurance/portfolios/${portfolio.id}/review`), "Portfolio review recorded")}>Record review</button>}</div>{!portfolio.sla.healthy && <p className="mt-1">{portfolio.sla.exceptions.map((item) => item.type.replaceAll("_", " ")).join(" · ")}</p>}</div>}
