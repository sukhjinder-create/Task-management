    ...
    <div className="max-w-2xl mx-auto py-8 px-4">
        ...
        {STEPS.map((label, i) => (
          <div key={i} className="flex items-center gap-2">
            <div className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full border
              ${i === step ? "border-[color:var(--primary)] text-[color:var(--primary)] bg-[var(--surface-soft)]" : i < step ? "border-[color:var(--border)] text-[color:var(--primary)]" : "border-[color:var(--border)] text-[color:var(--text-soft)]"}`}>
              {i < step && <CheckCircle size={12} />}
              {label}
            </div>
            {i < STEPS.length - 1 && <ChevronRight size={14} className="text-[color:var(--text-soft)] shrink-0" />}
          </div>
        ))}
      </div>
      ...
      {step === 0 && (
        <div className="space-y-6">
          <div className="theme-surface border theme-border rounded-xl p-5 space-y-4">
            ...
            <ol className="space-y-3 text-sm theme-text-muted list-none">
              <li className="flex gap-3">
                <span className="shrink-0 w-5 h-5 rounded-full border border-[color:var(--border)] text-[color:var(--primary)] text-xs font-bold flex items-center justify-center">1</span>
                <span>Go to <a href="https://api.slack.com/apps" target="_blank" rel="noreferrer" className="text-[color:var(--primary)] underline inline-flex items-center gap-1">api.slack.com/apps <ExternalLink size={11} /></a> and click <strong>Create New App → From scratch</strong>.</span>
              </li>
              <li className="flex gap-3">
                <span className="shrink-0 w-5 h-5 rounded-full border border-[color:var(--border)] text-[color:var(--primary)] text-xs font-bold flex items-center justify-center">2</span>
                <span>Give it any name (e.g. <em>Migration Bot</em>) and pick your Slack workspace.</span>
              </li>
              <li className="flex gap-3">
                <span className="shrink-0 w-5 h-5 rounded-full border border-[color:var(--border)] text-[color:var(--primary)] text-xs font-bold flex items-center justify-center">3</span>
            ...
            <div className="rounded-lg border theme-border overflow-hidden">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[var(--surface-soft)] border-b theme-border">
                ...
                <tbody>
                  {REQUIRED_SCOPES.map((s) => (
                    <tr key={s.scope} className="border-b theme-border last:border-0">
                      <td className="px-3 py-2 font-mono text-xs text-[color:var(--primary)]">{s.scope}</td>
            ...
            <ol className="space-y-3 text-sm theme-text-muted list-none" start={4}>
              <li className="flex gap-3">
                <span className="shrink-0 w-5 h-5 rounded-full border border-[color:var(--border)] text-[color:var(--primary)] text-xs font-bold flex items-center justify-center">4</span>
                <span>Click <strong>Install to Workspace</strong> and approve the permissions.</span>
              </li>
              <li className="flex gap-3">
                <span className="shrink-0 w-5 h-5 rounded-full border border-[color:var(--border)] text-[color:var(--primary)] text-xs font-bold flex items-center justify-center">5</span>
                <span>Copy the <strong>Bot User OAuth Token</strong> (starts with <code className="text-xs bg-[var(--surface-soft)] px-1 rounded">xoxb-</code>).</span>
              </li>
            </ol>
          </div>
          ...
          <button
            onClick={() => setStep(1)}
            className="w-full py-2.5 px-4 bg-[var(--primary)] hover:opacity-90 text-[color:var(--primary-contrast)] font-medium rounded-lg text-sm transition-opacity"
          >
            I have my token →
          </button>
        </div>
      ...
      {step === 1 && (
        <div className="space-y-5">
          <div className="theme-surface border theme-border rounded-xl p-5 space-y-4">
            ...
            <p className="text-sm theme-text-muted">Paste the Bot User OAuth Token from your Slack app. It should start with <code className="text-xs bg-[var(--surface-soft)] px-1 rounded">xoxb-</code>.</p>
            ...
            {validateError && (
              <div className="flex items-start gap-2 text-sm text-red-400 border border-red-800 rounded-lg px-3 py-2.5">
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep(0)} className="px-4 py-2.5 border theme-border rounded-lg text-sm theme-text hover:bg-[var(--surface-soft)] transition-colors">
              Back
            </button>
            <button
              onClick={handleValidate}
              disabled={!token.startsWith("xoxb-") || validating}
              className="flex-1 py-2.5 px-4 bg-[var(--primary)] hover:opacity-90 disabled:opacity-50 text-[color:var(--primary-contrast)] font-medium rounded-lg text-sm transition-opacity flex items-center justify-center gap-2"
            >
      ...
      {step === 2 && preview && (
        <div className="space-y-5">
          ...
          <div className="theme-surface border theme-border rounded-xl p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex gap-2 text-xs">
                <button onClick={() => setSelectedChannels(preview.channels.map((c) => c.id))} className="text-[color:var(--primary)] hover:underline">All</button>
                <span className="theme-text-muted">·</span>
                <button onClick={() => setSelectedChannels([])} className="text-[color:var(--primary)] hover:underline">None</button>
              </div>
            </div>
            <div className="space-y-1 max-h-64 overflow-y-auto">
              {preview.channels.map((ch) => (
                <label key={ch.id} className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-[var(--surface-soft)] cursor-pointer transition-colors">
                  <input
                    type="checkbox"
                  ...
                  />
                  <Hash size={13} className="text-[color:var(--text-soft)] shrink-0" />
                  <span className="text-sm theme-text">{ch.name}</span>
                  {ch.isArchived && <span className="text-[10px] border border-[color:var(--border)] text-[color:var(--text-muted)] px-1.5 py-0.5 rounded">archived</span>}
          ...
          <div className="border border-amber-800 rounded-xl p-4 text-sm text-amber-400">
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep(1)} className="px-4 py-2.5 border theme-border rounded-lg text-sm theme-text hover:bg-[var(--surface-soft)] transition-colors">
              Back
            </button>
            <button
      ...
      {step === 3 && migrateResult && (
        <div className="space-y-5">
          <div className="border border-[color:var(--border)] rounded-xl p-6 text-center">
            <CheckCircle size={40} className="mx-auto mb-3 text-[color:var(--primary)]" />
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="theme-surface border theme-border rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-[color:var(--primary)]">{migrateResult.usersCreated}</div>
              <div className="text-xs theme-text-muted">Users created</div>
            </div>
            <div className="theme-surface border theme-border rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-[color:var(--primary)]">{migrateResult.usersLinked}</div>
              <div className="text-xs theme-text-muted">Users linked</div>
            </div>
            <div className="theme-surface border theme-border rounded-xl p-4 text-center">
              <div className="text-2xl font-bold text-[color:var(--primary)]">{migrateResult.channelsCreated}</div>
              <div className="text-xs theme-text-muted">Channels created</div>
            </div>
          </div>

          <div className="theme-surface border theme-border rounded-xl p-4 text-center">
            <div className="text-3xl font-bold text-[color:var(--primary)]">{migrateResult.messagesImported}</div>
          ...
          <a
            href="/chat"
            className="block w-full py-2.5 px-4 bg-[var(--primary)] hover:opacity-90 text-[color:var(--primary-contrast)] font-medium rounded-lg text-sm transition-opacity text-center"
          >
            Go to Team Chat →
          </a>
        </div>
